...
function sortPaddockTypeByTotalArea() {
  const paddockTypeJoinPaddocks = paddockType.map(type => ({
    type,
    metros: paddocks
  ...
  return paddockTypeJoinPaddocks
    .sort((a, b) => b.metros - a.metros)
    .map((paddock) => paddock.type.name);
}
...
function sortFarmManagerByAdminArea() {
  const managersJoinPaddocks = paddockManagers.map(manager => ({
    manager,
    metros: paddocks
      .filter(paddocks => paddocks.paddockManagerId === manager.id)
      .reduce((total, { area }) => total + area, 0),
  }));

  return managersJoinPaddocks
    .sort((a, b) => b.metros - a.metros)
    .map((managers) => managers.manager.name);
}
...
function farmManagerNames() {
  const farmObject = farms.map(farm => {
    ...
    const farmManagers = paddockManagers
      .filter(manager => uniqueId.includes(manager.id))
      .sort((a, b) => a.name.localeCompare(b.name))
      .map(manager => manager.taxNumber);

    return { [farm.name]: farmManagers };
  });

  return Object.assign({}, ...farmObject);
}
...
function biggestAvocadoFarms() {
  const PADDOCK_PALTOS_ID = 1;

  const reducer = (paddocks, paddrock) => {
    const total = paddocks[paddrock.farmId] || 0;
    return { ...paddocks, [paddrock.farmId]: total + paddrock.area }
  }

  const paddocksByType = paddocks
    .filter(paddock => paddock.paddockTypeId === PADDOCK_PALTOS_ID)
    .reduce(reducer, {});

  return Object.values(paddocksByType)
...
function biggestCherriesManagers() {
  // CODE HERE
  const PADDOCK_CEREZAS_ID = 3;
  const PADDOCK_FARM_ID = 3;

  const reducer = (paddocks, paddrock) => {
    const name = paddockManagers.find(manager => manager.id === paddrock.paddockManagerId).name;
    const total = paddocks[name] || 0;
    return { ...paddocks, [name]: total + paddrock.area }
  }
  ...
  const paddocksByType = paddocks
    ...
    (
      paddock =>
        paddock.farmId === PADDOCK_FARM_ID &&
        paddock.paddockTypeId === PADDOCK_CEREZAS_ID
    )
    .reduce(reducer, {});

  return Object.entries(paddocksByType)
    .filter(p => p[1] > 1000)
...
function farmManagerPaddocks() {
  const managerObject = paddockManagers.map(manager => {
    const uniqueId = [...new Set([...
      paddocks
    ...
    const farmManagers = farms
      .filter(farm => uniqueId.includes(farm.id))
      .sort((a, b) => a.name.localeCompare(b.name))
      .map(farm => farm.name);

    return { [manager.name]: farmManagers };
  });

  return Object.assign({}, ...managerObject);
}
...
function paddocksManagers() {
  ...
  const joinPaddocks = paddocks.map(paddock => ({
    paddock,
    type: paddockType.find(type => type.id === paddock.paddockTypeId),
    manager: paddockManagers.find(manager => manager.id === paddock.paddockManagerId)
  }));

  const paddockObject = joinPaddocks.map(p => {
    const farmManager = { [p.manager.id]: p.manager.name };

    return { [`${p.type.name}-${p.paddock.harvestYear}`]: farmManager };
  });

  return Object.assign({}, ...paddockObject);
}
...
function newManagerRanking() {
  ...
  return sortFarmManagerByAdminArea().indexOf('FELIPE FARMER') + 1;
}
